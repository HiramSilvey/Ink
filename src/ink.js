...
function Item() {
	...
	for(var t in this.subItems){
	    for(var i in this.subItems[t]){
		ans += t + ": " + this.subItems[t][i].currentState.subtext + "\n";
	    }
	}
	return ans;
    }
...
function applyAction(action, item) {
	...
	for (var t in affectedItems) {
	    for(var i in affectedItems[t]){
		applyAction(event.action, affectedItems[t][i]);
	    }
	}
    }
}
