...
class ActionQuery {
  ...
  execute(context, subject, object) {
    ...
    for (let ao of action_object) {
      dlog('ACTION', `${as.name} ${this.verb} ${ao.name}`);
...
class Action { // lawsoot lawl
  ...
  execute(context, subject, verb, object) {
    ...
    // TODO(hiramj): subject and object will be reattained in the action query -- redundant
    return {
...
let dictionary = {
  ...
  "look": new DictionaryEntry(
    ...
    new ActionQuery("look",
      null, // the subject is just the unqualified subject
      ["or", ["sibling"],
        ["hasChild", ["hasName", "$subject"]]
      ])), // Only see the container and things that are in it
  "eat": new DictionaryEntry(
    ["devour", "gobble"],
    new ActionQuery("eat",
      null, // the subject is just the unqualified subject
      ["singleton", ["and", ["hasDescription", "$object"],
        ["owned"]
